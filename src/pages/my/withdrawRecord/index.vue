...
import icon_record_sh from '~/assets/images/icons/icon_record_sh.png'
...
async function fetchGetUserWithdrawRecord() {
  state.loading = true
  const res = await getUserWithdrawRecord({
    status: state.status,
...
<template>
  <div class="w-full">
    ...
    <van-list
      ...
      @load="handleLoadPage"
    >
      <div v-for="(item, index) in state.recordList" :key="index" class="mt-[8px] w-full border-b-solid border-light bg-white p-[20px]" @click="toPage(`/my/withdrawRecord/detail?order_id=${item.order_number}`)">
        <div class="flex-between-center">
          <div class="mr-[6px]">
            <img :src="icon_record_sh" class="w-[36px]" alt="" />
          </div>
          <div class="flex-col-center-start flex-auto">
            <span class="mb-[6px]">贖回</span>
          ...
          <div class="mr-[10px] flex-col-center-end">
            ...
            <span class="text-sm text-assist8">{{ withdrawStatusType[item.status] }}</span>
          </div>
