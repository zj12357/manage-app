    ...
    <div class="w-full">
      <div class="w-full bg-white p-[20px]">
        ...
        <div v-if="state.detail?.status === 3" class="w-full flex-col-center">
          ...
          <span class="my-[14px] font-500 text-assist3">申請中</span>
        </div>
      </div>

      <div class="mt-[8px] w-full bg-white">
        <van-cell-group>
          <van-cell title="類型" value="贖回" />
          <van-cell title="贖回金額" :value=" handleMoneySymbol(state.detail?.money)" />
          ...
          <van-cell title="狀態" :value="withdrawStatusType[state.detail?.status]" />
          <van-cell title="贖回時間" :value="state.detail?.created_at" />
        </van-cell-group>
      </div>
    </div>
  </div>
